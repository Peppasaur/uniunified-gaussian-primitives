...
void initKDTree(KDNode* root,int N,int* all_indices){
    float3 bmin{0,0,0};
    float3 bmax{1,1,1};
...
void buildKDTree(const BoundingBox* bboxes, int N, int max_depth,KDNode*& d_nodes,int*& all_indices) {
    ...
    printf("afterMalloc2\n");
    initKDTree(d_nodes,N,all_indices);
    printf("afterMalloc3\n");
...
void CreateScene(float* d_positions, float* d_orientations, float* d_scales,float* d_covs,int N,float* rgbs,int res,float* env_rgbs){
    ...
    KDNode* d_nodes;
    int* all_indices;
    buildKDTree(d_bboxes,N,18,d_nodes,all_indices);
    renderScene(d_positions, d_orientations, d_scales,d_covs,d_bboxes,N,d_nodes,15,4,res,1,rgbs,env_rgbs);
